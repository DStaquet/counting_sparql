...
def _delete_temp_dir(
    query_dir: str, q_query_object: Query
) -> None:
    temp_dir = get_query_output_dir(
        query_dir, q_query_object, temp_dir=True
    )
    if exists(temp_dir):
...
def _setup_tables(query: str, output_dir: str) -> None:
    ...
    query_output_dir = get_query_output_dir(
        output_dir, q_query_object, temp_dir=True
    )
...
def _setup_one_file(query: str, query_dir: str) -> None:
    ...
    entire_query = entire_run_query(
        q_query_object.algebra, sql_queries
    ) + entire_run_query(
        q_query_object.algebra, sql_delta_queries
    )
    with open(
        join(query_output_dir, "entire_query.sql"),
        "w",
        encoding="utf-8",
    ) as handle:
