...
def find_all_deltas(
    ...
    if file_format == "csv":
        return sorted(
            [
                ...
                if "delta" in file and file_format in file
            ],
    ...
    for file in os.listdir(delta_dir):
        if "insert" in file and file_format in file:
            ins_list.append(join(delta_dir, file))
        if "delete" in file and file_format in file:
...
def find_all_nus(
    ...
    return sorted(
        [
            join(delta_dir, file)
            for file in os.listdir(delta_dir)
            if "nu" in file and file_format in file
        ],
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Module to run SPARQL queries for benchmark"
        + " with incremental view maintenance on SQL queries.",
        prog="main.py",
        epilog="Query files first need to be set up with build_data.py,"
        + " can also be passed through this module as flags.",
    )
    parser.add_argument(
    ...
    if not args.multiple:
        ...
        delta_list = find_all_deltas(
            args.delta, file_format="csv"
        )
        nu_list = find_all_nus(
            args.nu_file, file_format="csv"
        )
    ...
    if args.init_csv:
        with open(
            args.output_csv, "w", encoding="utf-8"
        ) as f:
