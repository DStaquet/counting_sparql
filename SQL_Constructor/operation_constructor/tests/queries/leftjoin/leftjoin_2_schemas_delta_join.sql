...
CREATE TEMP TABLE delta_LeftJoin_7202945294355753096_schema_5974201903695169563_temp_1 AS
SELECT (CASE
            WHEN R1.x NOT NULL THEN R1.x
            ELSE R2.x
        END) AS x,
...
CREATE TEMP TABLE delta_LeftJoin_7202945294355753096_schema_5974201903695169563_2 AS
SELECT s1.x AS x,
       s1.k_count AS k_count
FROM nu_BGP_747695058721694287 AS s1
...
SELECT (CASE
       ...
       (CASE
            ...
            ELSE R1.k_count + R2.k_count
        END) AS k_count
FROM delta_LeftJoin_7202945294355753096_schema_5974201903695169563_temp_1 AS R1
FULL OUTER JOIN delta_LeftJoin_7202945294355753096_schema_5974201903695169563_2 AS R2 ON R1.x = R2.x
WHERE coalesce(R1.k_count, 0) + coalesce(R2.k_count, 0) != 0;